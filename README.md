...
WebGL is opt-in because each canvas holds its own GL context, and browsers evict the oldest once you exceed a low cap. The collection grid renders every card through the 2D path, so the collection of twenty-five studies costs zero GL contexts. Detail pages mount exactly one.
